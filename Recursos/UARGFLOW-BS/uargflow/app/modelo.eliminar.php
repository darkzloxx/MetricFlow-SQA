...
<html>
    <head>
        ...
        <title><?php echo Constantes::NOMBRE_SISTEMA; ?> - Eliminar Modelo</title>
    </head>
    <body>
        <?php include_once '../gui/navbar.php'; ?>
        <div class="container">
            <form action="modelo.eliminar.procesar.php" method="post">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <?php 
                        $query = "select * from metrica_modelo_calidad where id_modelo = ". $_GET["id"]; 
                        $consulta = BDConexion::getInstancia()->query($query);
                        $tiene = 1;
                        if ($consulta->num_rows > 0){
                            $tiene = 0;
                                echo "<p>El modelo tiene metricas asociadas, no puede ser eliminado</p>";
                            } else {
                        
                            $proyectos = "SELECT * FROM modelo_calidad where id_modelo = ". $_GET["id"]; 
                        ...
                        <p>¿Est&aacute; seguro que desea eliminar el Modelo <b><?= $Proyec['nombre']; ?></b>?</p>
                        <?php } }?>
                    </div>
                    <div class="card-footer">
                        ...
                        if ($tiene == 0){
                            ...
                            <button type='button' class='btn btn-outline-danger'>
                                <span class='oi oi-x'></span> Salir de esta pantalla
                            </button>
                            </a>";
                        } 
                        else {
