...
class PDF extends FPDF {
    ...
    // Sección de datos generales con ancho total coherente al de las tablas (270mm)
    function seccionProyecto($nombre, $fecha, $kpis = [], $filtrosTexto = 'Fase: Todas | Iteraciones: Todas | Métricas: Todas') {
        // Config visual
        $this->SetLineWidth(0.2);

        // Encabezado azul
        $this->SetFillColor(13, 110, 253); // rgb(13,110,253)
        $this->SetTextColor(255);
        $this->SetFont('Arial', 'B', 11);
        $this->Cell(0, 8, utf8_decode('DATOS GENERALES DEL PROYECTO'), 0, 1, 'C', true);

        // Preparación de anchos (total = 270)
        $TOTAL = 270.0;
        $half  = 135.0;
        $lblGray = [245,245,245];
        ...
        $L1 = 70; $V1 = $half - $L1; // 65
        $V1 = max(60, $V1); // asegurar un mínimo
        $pairLeft = $L1 + $V1; // puede exceder half si ajustado; corrige con right
        $R1 = 50; $VR1 = $TOTAL - $pairLeft - $R1; if ($VR1 < 30) { $VR1 = 30; $V1 = $TOTAL - ($L1 + $R1 + $VR1); }

        $this->SetFillColor($lblGray[0],$lblGray[1],$lblGray[2]);
        $this->cellTextEllipsized($L1, 7, 'Nombre del Proyecto:', 1, 0, 'L', true);
        $this->cellTextEllipsized($V1, 7, $nombre, 1, 0, 'L', false);
        $this->SetFillColor($lblGray[0],$lblGray[1],$lblGray[2]);
        $this->cellTextEllipsized($R1, 7, 'Fecha del Informe:', 1, 0, 'L', true);
        $this->cellTextEllipsized($VR1, 7, $fecha, 1, 1, 'L', false);

        // Row 2: Resumen de filtros (ocupa todo el ancho)
        $this->SetFillColor($lblGray[0],$lblGray[1],$lblGray[2]);
        $this->cellTextEllipsized(60, 7, 'Filtros aplicados:', 1, 0, 'L', true);
        $this->cellTextEllipsized($TOTAL-60, 7, $filtrosTexto, 1, 1, 'L', false);

        // Row 3: KPIs (dos pares)
        $pair = $half; $L = 80; $V = $pair - $L; if ($V < 35) $V = 35;
        $this->SetFillColor($lblGray[0],$lblGray[1],$lblGray[2]);
        $this->cellTextEllipsized($L, 7, 'Métricas planificadas:', 1, 0, 'L', true);
        $this->Cell($V, 7, (string)($kpis['metricas_planificadas'] ?? 0), 1, 0, 'C');
        $this->SetFillColor($lblGray[0],$lblGray[1],$lblGray[2]);
        $this->cellTextEllipsized($L, 7, 'Tipos de métricas distintas:', 1, 0, 'L', true);
        $this->Cell($V, 7, (string)($kpis['metricas_distintas'] ?? 0), 1, 1, 'C');

        // Row 4: Iteraciones con métricas (izquierda) + vacío (derecha)
        $this->SetFillColor($lblGray[0],$lblGray[1],$lblGray[2]);
        $this->cellTextEllipsized($L, 7, 'Iteraciones con métricas:', 1, 0, 'L', true);
        $this->Cell($V, 7, (string)($kpis['iteraciones_con_metricas'] ?? 0), 1, 0, 'C');
    ...
    function tablaResultados($data) {
        ...
        foreach ($data as $row) {
            ...
            if ($iterKey !== $currentIter) {
                ...
                $this->Cell(
                    0, 9,
                    utf8_decode("{$row['iteracion']}  ({$row['inicio']} - {$row['fin']})"),
                    0, 1, 'C', true
                );
...
// Construir WHERE dinámico compartido
$where = ['pf.id_proyecto=?','i.id_proyecto=?'];
$params = [$proyectoId,$proyectoId]; $types='ii';
if (!empty($fases)) {
    $place = implode(',', array_fill(0, count($fases), '?'));
    $where[] = 'f.nombre IN ('.$place.')';
    foreach($fases as $fa){ $params[]=$fa; $types.='s'; }
}
if (!empty($itersInput)) {
    ...
    foreach ($itersInput as $it) {
        ...
        if ($num !== null) { $sub[]='(f.nombre=? AND i.numero_iteracion=?)'; $params[]=$fn; $types.='s'; $params[]=$num; $types.='i'; }
    }
...
if (!empty($metricIds)) {
    ...
    foreach($metricIds as $mid){ $params[]=$mid; $types.='i'; }
}
...
function norm_date($v){if(!$v)return'';if(preg_match('/^(\d{4})-(\d{2})-(\d{2})/',$v,$m))return"{$m[3]}/{$m[2]}/{$m[1]}";return$v;}

$rows=[];
while($row=$res->fetch_assoc()){
    $plan=(float)$row['planificado'];$ejec=(float)$row['ejecutado'];
    if($plan==$ejec){$pct=100;$nota='Se cumplió';}
    elseif($plan==0&&$ejec>0){$pct=100;$nota='Se planificó 0 ('.$ejec.')';}
    elseif($ejec>$plan){$pct=($plan>0)?(int)round(($ejec/$plan)*100):100;$nota='Supera planificado (+'.($ejec-$plan).')';}
    elseif($plan>$ejec){$pct=($plan>0)?(int)round(($ejec/$plan)*100):0;$nota='Falta (-'.($plan-$ejec).')';}
    else{$pct=0;$nota='';}
    $rows[]=[
