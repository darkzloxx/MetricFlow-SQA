...
if ($proyectoId > 0) {
  ...
  if (!$esAdminGlobal && !ControlAcceso::usuarioPerteneceAProyecto($proyectoId)) {
    http_response_code(403); echo 'Acceso denegado al proyecto'; exit;
  }
}

$dataUrl = isset($_POST['image']) ? (string)$_POST['image'] : '';
if ($dataUrl === '') { http_response_code(400); echo 'Falta imagen'; exit; }
...
if ($data === false) { http_response_code(400); echo 'Imagen inválida'; exit; }
