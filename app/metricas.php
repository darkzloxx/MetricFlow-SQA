...
if ($esAdminGlobal || $esSuperAdmin) {
    ...
    foreach ($proyectosAccesibles as &$px) {
        ...
        if ($rsB = $cn->query($sqlMetB)) {
            while ($r = $rsB->fetch_assoc()) { $px['metricas'][(int)$r['id_metrica']] = $r; }
        }
        ...
        if ($rsPM = $cn->query('SELECT id_proyecto_modelo FROM proyecto_modelo_calidad WHERE id_proyecto = '.$idProyecto)) {
            while ($rr = $rsPM->fetch_assoc()) { $idsPM[] = (int)$rr['id_proyecto_modelo']; }
        }
        if (!empty($idsPM)) {
            $inPM = implode(',', array_filter($idsPM, function($v){ return $v>0; }));
            $sqlMetP = $hasTipo
            ...
            if ($rsP = $cn->query($sqlMetP)) {
                while ($r = $rsP->fetch_assoc()) { $px['metricas'][(int)$r['id_metrica']] = $r; }
            }
        }
    ...
    <div class="card">
        ...
        <div class="card-body">
            <?php if ($esAdminGlobal || $esSuperAdmin): ?>
                ...
                <?php endif; ?>
            <?php else: ?>
                <?php if (empty($proyectosAccesibles) || $iteracionesUsuario === 0): ?>
                ...
                <?php else: ?>
                    <?php foreach ($proyectosAccesibles as $px): ?>
                        <div class="mb-3">
                            <h5 class="mb-2">Proyecto: <strong><?= htmlspecialchars($px['proyecto']); ?></strong> — Modelo: <span class="badge badge-info"><?= htmlspecialchars($px['modelo']); ?></span></h5>
