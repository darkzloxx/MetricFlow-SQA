...
try {
    ...
    // Validaciones de formato: solo letras y espacios (con acentos y Ñ)
    $onlyLetters = '/^[A-Za-zÁÉÍÓÚáéíóúÑñ ]+$/u';
    if (!preg_match($onlyLetters, $nombre)) {
        throw new Exception('El nombre del modelo solo puede contener letras y espacios.');
    }
    if (!preg_match($onlyLetters, $descripcion)) {
        throw new Exception('La descripción del modelo solo puede contener letras y espacios.');
    }
    ...
    if (is_array($newNames) && count($newNames)) {
        ...
        foreach ($newNames as $idx => $nm) {
            ...
            if ($nm === '') continue;
            if (!preg_match($onlyLetters, $nm)) {
                throw new Exception('Nombre de nueva métrica inválido: "' . $nm . '". Use solo letras y espacios.');
            }
